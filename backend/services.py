...
async def get_current_user(db: orm.Session = fastapi.Depends(get_db), token: str = fastapi.Depends(oauth2schema)):
    try:
        ...
        raise fastapi.HTTPException(status_code=401, detail="Correo o clave no valido")
    
    return schemas.User.from_orm(user)

async def create_persona(user: schemas.User, db: orm.Session, persona: schemas.PersonaCreate):
    persona = models.Persona(**persona.dict(), usuario_id=user.id)
    db.add(persona)
    db.commit()
    db.refresh(persona)
...
async def get_personas(user: schemas.User, db: orm.Session):
    personas = db.query(models.Persona).filter_by(usuario_id=user.id)
    
    return list(map(schemas.Persona.from_orm, personas))

async def persona_selector(persona_id: int, user: schemas.User, db: orm.Session):
    persona = db.query(models.Persona).filter_by(usuario_id=user.id).filter(models.Persona.id == persona_id).first()
    
    if persona is None:
        raise fastapi.HTTPException(status_code=404, detail="La persona no existe")

    return persona

async def get_persona(persona_id: int, user: schemas.User, db: orm.Session):
    persona = await persona_selector(persona_id=persona_id, user=user, db=db)
    
    return schemas.Persona.from_orm(persona)

async def delete_persona(persona_id: int, user: schemas.User, db: orm.Session):
    persona = await persona_selector(persona_id, user, db)
    
    db.delete(persona)
    db.commit()
    
async def update_persona(persona_id: int, persona: schemas.PersonaCreate, user: schemas.User, db: orm.Session):
    persona_db = await persona_selector(persona_id, user, db)
